...
public class GameManager : MonoBehaviour
{
    ...
    public void PlayerSwitchesWorld(int _clientId, int _portalId)
    {
        if (_clientId == Client.instance.myId)
        {
            DungeonManager.instance.LoadDungeon((Dungeons) _portalId, _clientId);
        }
        else
        {
            if (players.ContainsKey(_clientId))
            {
